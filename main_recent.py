...
def load_models():
    ...
    # Model 1: NLI Classifier
    nli_model_name = "MoritzLaurer/mDeBERTa-v3-base-mnli-xnli"
    classifier = pipeline("zero-shot-classification", model=nli_model_name, device=device, multi_label=True)
...
def nuanced_clean_text(text):
    ...
    for better sentiment and sarcasm detection.
    """
    text = str(text)
    
    # The powerful NLI model gets the text with minimal cleaning.
    # We only remove URLs and emojis which provide little value to it.
    original_for_nli = re.sub(r"http\S+", "", text)

    # ### --- UPDATED EMOJI REMOVAL PATTERN --- ###
    # This is a more comprehensive and aggressive pattern to remove all emojis.
    ...
    # For rules, we do more aggressive cleaning.
    text_for_rules = text.lower()
    text_for_rules = re.sub(r"http\S+", "", text_for_rules)
    text_for_rules = emoji_pattern.sub(r'', text_for_rules) # Also clean emojis for rules
    
    # Replace punctuation with a space instead of just removing it.
...
def check_hate_speech(text, hate_speech_phrases):
    # Hate speech check should be case-insensitive
...
def process_bulk_data_combined(df, text_col, candidate_labels, hate_speech_phrases, rules_df, label_to_parent_map, discovery_mode=False):
    ...
    progress_bar.progress(0.5, text=f"Step 3/3: Processing {len(texts_to_process)} rows with AI model...")

    if texts_to_process:
        if discovery_mode:
            # Discovery mode logic here...
            pass 
        else: # Classification Mode
            batch_results = classifier(texts_to_process, candidate_labels)
            for label in candidate_labels: df[f"score_{label}"] = 0.0
            ...
            for i, result_dict in enumerate(batch_results):
                ...
                for label, score in zip(result_dict['labels'], result_dict['scores']):
                    df.loc[original_index, f"score_{label}"] = score
    ...
    cols = ['original_text', 'top_label', 'parent_label', 'top_score', 'cleaned_text_for_rules', 'text_for_nli'] + [c for c in df.columns if c not in ['original_text', 'top_label', 'parent_label', 'top_score', 'cleaned_text_for_rules', 'text_for_nli']]
    df = df[cols]
...
def main():
    ...
    # Using the refined labels we developed
